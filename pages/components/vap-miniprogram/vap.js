(function (global, factory) {
  ...
  var runtime = (function (exports) {
    ...
    // object. Either way, the resulting object will be used tialize
  ...
  ***************************************************************************** */
  ...
  function resetCanvasQuality(ctx, { width = 0, height = 0, dpr = 1}) {
  ...
  var FrameParser = /*#__PURE__*/function () {
    ...
    createClass(FrameParser, [{
      ...
      value: function loadImg(url) {
        ...
        var ctx = this.ctx
        resetCanvasQuality(ctx, this.headData)
  ...
  var VapVideo = /*#__PURE__*/function () {
    function VapVideo(options) {
      ...
      if (!this.options.src || !this.options.config) {
        console.error('参数出错：src(视频地址)、config(配置文件地址)');
      } else {
        // init video
        // this.initVideo();

        if (this.options.mode === 'decoder') {
          this.decoder = wx.createVideoDecoder()
        } else {
          this.video = this.options.v
        }
      }
    }
    ...
    createClass(VapVideo, [{
      ...
      value: function requestAnimFunc() {
        ...
        this.frame = 1
        return function (cb) {
          return me.options.c2.requestAnimationFrame(function (ts) {
            // console.log(me.frame, Math.floor(1000 / me.options.fps), ts - startTs)
            if (startTs === 0 || ts - startTs >= Math.floor(1000 / me.options.fps)) {
              startTs = ts
              return cb();
            }
      ...
      value: function cancelRequestAnimation() {
        if (this.animId) {
          this.options.c2.cancelAnimationFrame(this.animId)
        }
      }
    }, {
      key: "destroy",
  ...
  var WebglRenderVap = /*#__PURE__*/function (_VapVideo) {
    ...
    createClass(WebglRenderVap, [{
      ...
      value: function setCanvas() {
        ...
        if (!canvas) {
          canvas = this.instance.canvas = this.options.c2
        }
      ...
      value: function initWebGL() {
        ...
        if (!gl) {
          this.instance.gl = gl = canvas.getContext('webgl', {
            alpha: true,
            depth: true,
            stencil: true,
            antialias: true,
      ...
      value: function drawFrame() {
        ...
        if (this.vapFrameParser) {
          // var frame = Math.floor(this.video.currentTime * this.options.fps);  // 无法获取当前视频时间
          // var frame = Math.floor((ts - this.startTs) / 1000 * this.options.fps) - 1
          if (frame >= this.vapFrameParser.config.info.f) {
            this.destroy()
            return
          }
        ...
        var imageData = null
        if (this.options.mode === 'decoder') {
          imageData = this.decoder.getFrameData()
        } else {
          var c1 = this.vapFrameParser.canvas
          var ctx = this.vapFrameParser.ctx
          resetCanvasQuality(
            this.vapFrameParser.ctx,
            this.options
          )
          var w = c1.width
          var h = c1.height
        ...
        // 真机预览制定二维纹理失败
        // gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, imageData); // 指定二维纹理方式
  ...
  return (options) => new WebglRenderVap(Object.assign({}, options));
